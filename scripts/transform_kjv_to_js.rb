...
def open_and_transform(testament)
  ...
  Dir.glob("/Users/niklaslilland/dev/king-json-bible/formatted/#{testament}/*.json") do |file_name|
    ...
    book_folder = "/Users/niklaslilland/dev/king-json-bible/KJV/js/#{original_file_name_book}"
    Dir.mkdir(book_folder) unless File.directory?(book_folder)

    grouped.each do |chapter, contents|
      File.open("/Users/niklaslilland/dev/king-json-bible/KJV/js/#{original_file_name_book}/#{chapter}.js", 'w+') do |file|
        file.write("export default #{contents.to_json};")
